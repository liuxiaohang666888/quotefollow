...
exports.sanitizeMimeNoise = sanitizeMimeNoise;
exports.parseMimeMessage = parseMimeMessage;
...
function normalizeCharset(cs) {
    ...
    if (['big5', 'big5-hkscs', 'cn-big5', 'x-big5'].includes(c))
        return 'big5';
    if (['ks_c_5601-1987', 'euc-kr', 'ksc5601'].includes(c))
        return 'euc-kr';
    if (['shift-jis', 'shift_jis', 'sjis', 'cp932', 'ms932'].includes(c))
        return 'shift-jis';
...
function decodeRfc2047(input) {
    ...
    return merged.replace(/=\?([^?\s]+)\?([bBqQ])\?([^?\s]*)\?=/g, (all, cs, enc, data) => {
        try {
            let bytes;
...
function collectParts(headers, body, out, depth) {
...
function sanitizeMimeNoise(input) {
    if (!input)
        return input || '';
    if (!/NextPart|mimepart/i.test(input) &&
    ...
    for (const raw of input.split(/\r?\n/)) {
        ...
        if (/^(Content-[\w-]+|MIME-Version)\s*:/i.test(t))
            continue;
        if (/^This is a multi-part message in MIME format\.?$/i.test(t))
            continue;
        if (B64_LINE.test(t)) {
            b64Run++;
            continue;
        }
        if (b64Run > 0 && B64_FRAG.test(t))
            continue;
        b64Run = 0;
        out.push(raw);
    }
...
function tryDecodePart(p) {
    ...
    if (!ct.startsWith('text/') && !ct.startsWith('message/'))
        return null;
    if (ct.startsWith('text/') && ct.includes('calendar'))
        return null;
    try {
        let result;
...
function parseMimeMessage(rawLatin1) {
